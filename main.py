...
dans l'apprentissage.
  ...
  EXÉCUTION SIMPLE
  ----------------
  Lancer le fichier python projet4.py
  ...
  --------------------------------------------------------------------------
  COMMANDES (facultatives, pour les expériences chiffrées du rapport)
  ---------
    python projet4.py cliffwalking          expérience 2 : chemin risqué / chemin sûr
    python projet4.py frozenlake            expérience 1 : cartes 4x4 puis 8x8
    python projet4.py frozenlake --cartes 4x4
    python projet4.py epsilon               expérience 3 : fixe contre décroissant
    python projet4.py epsilon --balayage    valeurs de epsilon fixe de 0,3 à 0,01
    python projet4.py parcours              les deux démonstrations à la suite
    python projet4.py demo --comparer       animation des deux agents côte à côte
    python projet4.py demo --comparer --video resultats/demo.mp4
    python projet4.py rejouer --model resultats/cliffwalking_qlearning.npy
    python projet4.py tout                  enchaîne les trois expériences

  Ajouter --rapide à une expérience pour une exécution courte de vérification.
...
ENVIRONNEMENTS = {
    ...
    "cliffwalking": dict(
        # Gymnasium 1.2 a renommé l'environnement en v1. On garde v0 en secours
        # pour rester compatible avec les installations plus anciennes.
        candidats=["CliffWalking-v1", "CliffWalking-v0"],
        kwargs={},
        max_pas=200,
        # Ici toute fin d'épisode non tronquée signifie « arrivée atteinte » :
        # tomber dans la falaise ne termine pas l'épisode, cela renvoie au départ.
...
class QLearning(AgentTabulaire):
    ...
    Sur CliffWalking, l'agent apprend la valeur du chemin longeant la falaise sans
    jamais tenir compte du fait que son propre epsilon l'y fera tomber.
    """
    nom = "Q-learning"
...
def tracer_trajectoires_cliff(chemins, titres, fichier, lignes=4, colonnes=12):
    """Trajectoires apprises, avec la falaise grisée, le départ et l'arrivée annotés."""
    fig, axes = plt.subplots(len(chemins), 1, figsize=(9, 2.6 * len(chemins)))
    axes = [axes] if len(chemins) == 1 else list(axes)

    for ax, chemin, titre in zip(axes, chemins, titres):
        grille = np.zeros((lignes, colonnes))
        grille[lignes - 1, 1:colonnes - 1] = 1.0
        ax.imshow(grille, cmap="Greys", vmin=0, vmax=2)

        ys = [s // colonnes for s in chemin]
        xs = [s % colonnes for s in chemin]
        ax.plot(xs, ys, "-o", color="#2a78d6", lw=2, ms=5)
        ax.plot(xs[0], ys[0], "s", color="#1baf7a", ms=12)
        ax.plot(colonnes - 1, lignes - 1, "*", color="#d62a2a", ms=18)
        ax.text(0, lignes - 1.35, "D", ha="center", weight="bold")
        ax.text(colonnes - 1, lignes - 1.35, "A", ha="center", weight="bold")
        ax.set_title("{}  ({} pas)".format(titre, len(chemin) - 1), fontsize=11, weight="bold")

        # Le quadrillage doit passer ENTRE les cases, donc sur les ticks mineurs
        # décalés d'une demi-case. Sur les ticks majeurs, il couperait les cases.
        ax.set_xticks([]), ax.set_yticks([])
        ax.set_xticks(np.arange(-0.5, colonnes, 1), minor=True)
...
def experience_cliffwalking(args):
    """
    Expérience 2 : CliffWalking, cas d'école de la différence sur-politique /
    hors-politique. Répond au point 4 du sujet.

    Réglage volontairement proche de la figure 6.4 de Sutton & Barto : epsilon FIXE
    à 0,1, gamma = 1. Le epsilon fixe est essentiel, c'est lui qui fait payer à SARSA
    le coût de ses chutes accidentelles. Sutton prend alpha = 0,5 ; à cette valeur la
    table de SARSA reste assez bruitée pour que sa politique gloutonne boucle sur
    environ un quart des graines, d'où le passage à 0,25.
    """
    if args.rapide:
        args.episodes, args.graines = 300, 5

    graines = list(range(args.graines))
    hyper = dict(alpha=args.alpha, gamma=args.gamma, mode_epsilon="fixe",
                 epsilon_debut=args.epsilon, epsilon_fin=args.epsilon)

    print("\n=== CliffWalking | {} épisodes | {} graines | epsilon fixe {} ==="
          .format(args.episodes, len(graines), args.epsilon))

    resultats, lignes_tableau, chemins, titres = {}, [], [], []
    for algo in ALGOS:
        resultats[algo] = campagne("cliffwalking", algo, args.episodes, graines, hyper=hyper)
        r = resume_evaluations(resultats[algo])

        # Récompense PENDANT l'entraînement, sur le dernier quart des épisodes :
        # c'est la métrique de la figure 6.4, elle inclut les chutes dues à epsilon.
        fin_entrainement = resultats[algo]["recompenses"][:, int(0.75 * args.episodes):].mean()

        Q0 = resultats[algo]["tables"][0]
        chemin = trajectoire_gloutonne("cliffwalking", Q0)
        chemins.append(chemin), titres.append(NOMS[algo])

        print("   {:<16} entraînement {:>8.1f} | glouton {:>7.1f} ({:.0f} pas) | ligne {:.2f}"
              .format(NOMS[algo], fin_entrainement, r["retour_moy"],
                      r["longueur_moy"], ligne_moyenne(chemin)))
        lignes_tableau.append([
            NOMS[algo], "{:.1f}".format(fin_entrainement),
            "{:.1f} ± {:.1f}".format(r["retour_moy"], r["retour_std"]),
            "{:.1f}".format(r["longueur_moy"]), "{:.2f}".format(ligne_moyenne(chemin)),
            "{:.0f} %".format(r["succes_moy"])])

        np.save(fichier_resultat("cliffwalking_{}.npy".format(algo)), Q0)
        tracer_politique(Q0, 4, 12, "Politique gloutonne, {}".format(NOMS[algo]),
                         fichier_resultat("exp2_politique_{}.png".format(algo)),
                         etats_speciaux={**{s: "X" for s in FALAISE},
                                         DEPART: "D", ARRIVEE: "A"})

    fig, ax = plt.subplots(figsize=(8, 4.6))
    for algo in ALGOS:
        courbe_multi_graines(ax, resultats[algo]["recompenses"], NOMS[algo],
                             COULEURS[NOMS[algo]], fenetre=20)
    ax.set_ylim(-160, 0)
    ax.set_xlabel("Épisode"), ax.set_ylabel("Somme des récompenses")
    ax.set_title("CliffWalking, epsilon fixe = {} : récompense pendant l'apprentissage\n"
                 "moyenne sur {} graines, bande = ± 1 écart-type"
                 .format(args.epsilon, len(graines)), fontsize=11, weight="bold")
    ax.legend()
    fig.tight_layout()
    chemin = fichier_resultat("exp2_cliffwalking_recompenses.png")
    fig.savefig(chemin, dpi=140)
    plt.close(fig)
    print("   [figure]", chemin)

    tracer_trajectoires_cliff(chemins, titres, fichier_resultat("exp2_trajectoires.png"))
    sauver_tableau(["Algorithme", "Récompense pendant l'entraînement (dernier quart)",
                    "Retour glouton", "Longueur gloutonne", "Ligne moyenne du chemin",
                    "Arrivées atteintes"],
                   lignes_tableau, "exp2_cliffwalking_resume",
                   titre="CliffWalking, epsilon fixe = {}".format(args.epsilon))
...
def experience_epsilon(args):
    ...
    if chemins:
        tracer_trajectoires_cliff(chemins, titres, fichier_resultat("exp3_trajectoires.png"))
...
def figures_demo(nom_env, noms_agents, tables):
    ...
    speciaux = ({**{s: "X" for s in FALAISE}, DEPART: "D", ARRIVEE: "A"}
                if nom_env == "cliffwalking" else None)
    ...
    for nom_agent, Q in zip(noms_agents, tables):
        ...
        tracer_politique(Q, lignes, colonnes,
                         ...
                         fichier_resultat("demo_politique_{}_{}.png".format(nom_env, nom_agent)),
                         etats_speciaux=speciaux)
        if nom_env == "cliffwalking":
            chemins.append(trajectoire_gloutonne("cliffwalking", Q))
            titres.append(NOMS[nom_agent])

    if chemins:
        tracer_trajectoires_cliff(chemins, titres,
                                  fichier_resultat("demo_trajectoires_cliffwalking.png"))
...
def construire_parseur():
    p = argparse.ArgumentParser(
        description="Projet 4 : Q-learning contre SARSA sur FrozenLake et CliffWalking",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="Exemple : python projet4.py cliffwalking --rapide")
    sous = p.add_subparsers(dest="commande", required=True)

    c = sous.add_parser("cliffwalking", help="expérience 2 : chemin risqué contre chemin sûr")
    c.add_argument("--episodes", type=int, default=2000)
    c.add_argument("--graines", type=int, default=20)
    c.add_argument("--alpha", type=float, default=0.25)
    c.add_argument("--gamma", type=float, default=1.0)
    c.add_argument("--epsilon", type=float, default=0.1, help="epsilon fixe")
    c.add_argument("--rapide", action="store_true")
    c.set_defaults(fonction=experience_cliffwalking)
    ...
    d.add_argument("--env", default="cliffwalking", choices=list(DEMO))
...
def tout_lancer(args):
    ...
    for commande in ("cliffwalking", "frozenlake", "epsilon"):
...
def parcours(args=None):
    ...
    print("   python projet4.py cliffwalking")
    print("   python projet4.py frozenlake")
    print("   python projet4.py epsilon")
    print("   python projet4.py tout")
...
def main(argv=None):
    ...
    if not argv:
        ...
        print("   python projet4.py frozenlake | cliffwalking | epsilon | tout\n")
